...
<!-- 			<navigator url="../../pages/index/index" class="homeIcon-wrap">
			...
			<view class="content">
				<view class="txt">邀请码： {{ inviteCode }}</view>
		...
		<view class="goods">
			<view class="items" v-for="(item, index) in packageList" :key="index">
				<view class="good_img"><image :src="item.image" mode="aspectFit"></image></view>
...
export default {
	...
	onLoad(option) {
		...
		if (option) {
			this.inviteCode = decodeURIComponent(option.scene).split('=')[1];
			uni.setStorageSync('shareId',this.inviteCode)
		}
	},
	onShow() {
		const token = uni.getStorageSync('token');
		...
		if (token) {
			...
			this.$common.loginGetToken(this.inviteCode).then(res => {
				if (res) {
					this.getpackagelist(); // 调用方法
				}
			});
	...
	methods: {
		...
		handleBuy(index) {
			console.log(index);
			// this.$common.passUserauth(this.inviteCode).then(res => {
			// 	if (res) {
			...
			// 	} else {
			// 		uni.navigateTo({
			// 			url: `../auth/auth?shareId=${this.inviteCode}`
			// 		});
			// 	}
			// });
		},
			...
			.good_desc {
				...
				.good_price_wrap {
					...
					.good_btn {
						...
						line-height: 48rpx;
						background: rgba(255, 255, 255, 1);
						border: 1rpx solid rgba(28, 27, 27, 1);
						border-radius: 4rpx;
						text-align: center;
						text {
							width: 50rpx;
