...
<template>
	<view class="number-box">
		<block v-for="(myIndex, index) in indexArr" :key="index">
			<swiper class="swiper" vertical="true" :current="myIndex" circular="true" v-bind:style="{color:color,width:myIndex == 10 ? '7px' : _width,height:_height,lineHeight:_fontSize,fontSize:_fontSize}">
				<swiper-item>
...
<script>
	export default {
		...
		computed: {
			_width() {
				console.log(uni.upx2px(this.width) + 'px')
